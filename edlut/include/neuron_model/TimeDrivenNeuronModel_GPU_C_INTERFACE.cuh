...
class TimeDrivenNeuronModel_GPU_C_INTERFACE : public TimeDrivenModel {
	public:
		...
		*/
		virtual std::map<std::string, boost::any> GetSpecificNeuronParameters(int index) const  = 0;
		...
		* \throw EDLUTException If it happens a mistake with the parameters in the dictionary.
		*/
		virtual IntegrationMethod_GPU_C_INTERFACE * CreateIntegrationMethod(ModelDescription imethodDescription)  = 0;
		 ...
		 */
		template <class Neuron_Model>
		static ModelDescription ParseIntegrationMethod(FILE * fh, long & Currentline) {

...
cdef class PySimulation_API:
    ...
    def GetLayerState(self, neuron_id):
        """
        It retrieves the state of a set of neurons.

        :param neuron_id: Index of the neuron as returned by AddNeuronLayer function.
        :return: A tuple with 1) Numpy array with the state values of size <neurons,states> and 2) the neuron vector id to access the specific neuron.
        ...
        cdef int neuron_vector_id
        cdef int n_neurons
        cdef int n_states
        cdef float* cpp_state_ptr = self.cpp_sim.GetLayerState(neuron_id, neuron_vector_id, n_neurons, n_states)
        ...
        return np.asarray(cpp_state), neuron_vector_id

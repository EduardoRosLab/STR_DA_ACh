...
std::vector<int> Simulation_API::AddNeuronLayer(int num_neurons, std::string model_name, std::map<std::string, boost::any> param_dict,
                                    bool log_activity, bool output_activity) {
	try{
		if (this->initialized){
...
int Simulation_API::AddLearningRule(std::string model_name, std::map<std::string, boost::any> param_dict) {
...
std::vector<int> Simulation_API::AddSynapticLayer(const std::vector<int> & source_neuron, const std::vector<int> & target_neuron,
                                                  std::map<std::string, boost::any> param_dict) {
	try{
		if (this->initialized){
...
void Simulation_API::AddExternalSpikeActivity(const std::vector<double> & event_time, const std::vector<long int> & neuron_index) {
	try{
		if (!this->initialized) {
...
void Simulation_API::AddExternalCurrentActivity(const std::vector<double> & event_time, const std::vector<long int> & neuron_index, const std::vector<float> & current_value) {
	try{
		if (!this->initialized) {
...
void Simulation_API::GetSpikeActivity(std::vector<double> & event_time, std::vector<long int> & neuron_index) {
	try{
		if (!this->initialized) {
...
void Simulation_API::AddFileInputSpikeActivityDriver(string FileName) {
	try{
		if (this->initialized){
...
void Simulation_API::AddFileInputCurrentActivityDriver(string FileName) {
	try{
		if (this->initialized){
			throw EDLUTException(TASK_INITIALIZE_SIMULATION, ERROR_INITIALIZED_SIMULATION, REPAIR_EXECUTE_BEFORE_INITIALIZE_SIMULATION);
		}
...
void Simulation_API::AddFileOutputSpikeActivityDriver(string FileName) {
	try{
		if (this->initialized){
			throw EDLUTException(TASK_INITIALIZE_SIMULATION, ERROR_INITIALIZED_SIMULATION, REPAIR_EXECUTE_BEFORE_INITIALIZE_SIMULATION);
		}
...
void Simulation_API::AddFileOutputMonitorDriver(string FileName, bool monitor_state) {
	try{
		if (this->initialized){
			throw EDLUTException(TASK_INITIALIZE_SIMULATION, ERROR_INITIALIZED_SIMULATION, REPAIR_EXECUTE_BEFORE_INITIALIZE_SIMULATION);
		}
...
void Simulation_API::AddFileOutputWeightDriver(string FileName, float save_weight_period) {
	try{
		if (this->initialized){
			throw EDLUTException(TASK_INITIALIZE_SIMULATION, ERROR_INITIALIZED_SIMULATION, REPAIR_EXECUTE_BEFORE_INITIALIZE_SIMULATION);
		}
...
void Simulation_API::GetCompressedWeights(std::vector<int> & N_equal_weights, std::vector<float> & equal_weights) {
	try{
		if (!this->initialized) {
...
std::vector<float> Simulation_API::GetWeights() {
	try{
		if (!this->initialized) {
			throw EDLUTException(TASK_GET_WEIGHTS, ERROR_NON_INITIALIZED_SIMULATION, REPAIR_EXECUTE_AFTER_INITIALIZE_SIMULATION);
		}
...
std::vector<float> Simulation_API::GetSelectedWeights(std::vector<int> synaptic_indexes) {
	try{
		if (!this->initialized) {
			throw EDLUTException(TASK_GET_WEIGHTS, ERROR_NON_INITIALIZED_SIMULATION, REPAIR_EXECUTE_AFTER_INITIALIZE_SIMULATION);
		}
...
void Simulation_API::Initialize()  {
	try{
		if (this->initialized){
			throw EDLUTException(TASK_INITIALIZE_SIMULATION, ERROR_INITIALIZED_SIMULATION, REPAIR_EXECUTE_BEFORE_INITIALIZE_SIMULATION);
		}
...
void Simulation_API::SetSimulationParameters(std::map<std::string, boost::any> param_dict) {
	try{
		if (this->initialized){
...
void Simulation_API::RunSimulation(double end_simulation_time) {
	try{
		if (!this->initialized){
...
std::map<std::string, boost::any> Simulation_API::GetLearningRuleDefParams(std::string model_name) {
	try{
...
std::map<std::string, boost::any> Simulation_API::GetNeuronModelDefParams(std::string model_name) {
	try{
...
std::map<std::string, boost::any> Simulation_API::GetIntegrationMethodDefParams(std::string model_name) {
	try{
...
std::map<std::string, boost::any> Simulation_API::GetLearningRuleParams(int lrule_index) {
	try{
		if (!this->initialized){
...
void Simulation_API::SetLearningRuleParams(int lrule_index, std::map<std::string, boost::any> newParam) {
	try{
		if (!this->initialized){
...
float* Simulation_API::GetLayerState(int neuron_id, int& neuron_vector_id, int& n_neurons, int& n_states) {
	try{
		if (!this->initialized){
		...
		*/
		...
		auto state = this->simulation->GetNetwork()->GetNeuronAt(neuron_id)->GetVectorNeuronState();
		neuron_vector_id = this->simulation->GetNetwork()->GetNeuronAt(neuron_id)->GetIndex_VectorNeuronState();
		n_neurons = state->GetSizeState();
		n_states = state->GetNumberOfVariables();
...
std::vector<std::map<std::string, boost::any> > Simulation_API::GetNeuronParams(std::vector<int> neuron_index) {
	try{
		if (!this->initialized){
			throw EDLUTException(TASK_GET_NEURON_PARAMS, ERROR_NON_INITIALIZED_SIMULATION, REPAIR_EXECUTE_AFTER_INITIALIZE_SIMULATION);
		}
...
std::vector<std::map<std::string, boost::any> > Simulation_API::GetSpecificNeuronParams(std::vector<int> neuron_index) {
	try{
		if (!this->initialized){
			throw EDLUTException(TASK_GET_NEURON_PARAMS, ERROR_NON_INITIALIZED_SIMULATION, REPAIR_EXECUTE_AFTER_INITIALIZE_SIMULATION);
		}
...
void Simulation_API::SetSpecificNeuronParams(int neuron_index, std::map<std::string, boost::any> newParam) {
	try{
		if (!this->initialized){
...
std::map<std::string, std::string> Simulation_API::GetNeuronModelInfo(string neuronModelName) {
	try{
...
void Simulation_API::PrintNeuronModelInfo(string neuronModelName) {
	try{
...
std::map<std::string, std::string> Simulation_API::GetVectorizableParameters(string neuronModelName) {
	try{
...
void Simulation_API::PrintVectorizableParameters(string neuronModelName) {
	try{

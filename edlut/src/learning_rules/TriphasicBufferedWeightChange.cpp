...
ModelDescription TriphasicBufferedWeightChange::ParseLearningRule(FILE * fh)  {
...
void TriphasicBufferedWeightChange::SetParameters(std::map<std::string, boost::any> param_map) {
...
void TriphasicBufferedWeightChange::ApplyPostSynapticSpike(Neuron * neuron, double SpikeTime){
	...
	for (int j = 0; j < N_spikes; j++){
		...
		int tableIndex = abs(ElapsedTime + alpha)*this->N_elements*this->inv_maxTimeMeasured;
		neuron->GetInputConnectionWithPostSynapticLearningAt(LearningRuleId, spike_data[j].synapse_index)->IncrementWeight(this->a * this->kernelLookupTable[tableIndex]);
	}
}

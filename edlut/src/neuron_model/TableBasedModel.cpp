...
void TableBasedModel::LoadNeuronModel(string ConfigFile) {
	FILE *fh;
	long Currentline = 0L;
...
void TableBasedModel::LoadTables(string TableFile) {
	FILE *fd;
	unsigned int i;
...
void TableBasedModel::LoadNeuronModel() {
...
std::map<std::string, boost::any> TableBasedModel::GetSpecificNeuronParameters(int index) const {
	return GetParameters();
}

void TableBasedModel::SetParameters(std::map<std::string, boost::any> param_map) {
...
ModelDescription TableBasedModel::ParseNeuronModel(std::string FileName) {

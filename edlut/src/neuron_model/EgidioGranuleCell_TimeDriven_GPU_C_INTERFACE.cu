...
std::map<std::string, boost::any> EgidioGranuleCell_TimeDriven_GPU_C_INTERFACE::GetSpecificNeuronParameters(int index) const {
	return GetParameters();
}

void EgidioGranuleCell_TimeDriven_GPU_C_INTERFACE::SetParameters(std::map<std::string, boost::any> param_map) {
...
IntegrationMethod_GPU_C_INTERFACE * EgidioGranuleCell_TimeDriven_GPU_C_INTERFACE::CreateIntegrationMethod(ModelDescription imethodDescription) {
...
ModelDescription EgidioGranuleCell_TimeDriven_GPU_C_INTERFACE::ParseNeuronModel(std::string FileName) {
	FILE *fh;

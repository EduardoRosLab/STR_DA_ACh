...
std::map<std::string, boost::any> AdExTimeDrivenModel_GPU_C_INTERFACE::GetSpecificNeuronParameters(int index) const {
	return GetParameters();
}

void AdExTimeDrivenModel_GPU_C_INTERFACE::SetParameters(std::map<std::string, boost::any> param_map) {
...
IntegrationMethod_GPU_C_INTERFACE * AdExTimeDrivenModel_GPU_C_INTERFACE::CreateIntegrationMethod(ModelDescription imethodDescription) {
...
ModelDescription AdExTimeDrivenModel_GPU_C_INTERFACE::ParseNeuronModel(std::string FileName) {
	FILE *fh;

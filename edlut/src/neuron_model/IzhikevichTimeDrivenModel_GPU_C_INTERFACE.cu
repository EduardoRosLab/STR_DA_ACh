...
std::map<std::string, boost::any> IzhikevichTimeDrivenModel_GPU_C_INTERFACE::GetSpecificNeuronParameters(int index) const {
	return GetParameters();
}

void IzhikevichTimeDrivenModel_GPU_C_INTERFACE::SetParameters(std::map<std::string, boost::any> param_map) {
...
IntegrationMethod_GPU_C_INTERFACE * IzhikevichTimeDrivenModel_GPU_C_INTERFACE::CreateIntegrationMethod(ModelDescription imethodDescription) {
...
ModelDescription IzhikevichTimeDrivenModel_GPU_C_INTERFACE::ParseNeuronModel(std::string FileName) {
	FILE *fh;

...
std::map<std::string, boost::any> TimeDrivenPurkinjeCell_GPU_C_INTERFACE::GetSpecificNeuronParameters(int index) const {
	return GetParameters();
}

void TimeDrivenPurkinjeCell_GPU_C_INTERFACE::SetParameters(std::map<std::string, boost::any> param_map) {
...
IntegrationMethod_GPU_C_INTERFACE * TimeDrivenPurkinjeCell_GPU_C_INTERFACE::CreateIntegrationMethod(ModelDescription imethodDescription) {
...
ModelDescription TimeDrivenPurkinjeCell_GPU_C_INTERFACE::ParseNeuronModel(std::string FileName) {
	FILE *fh;

...
FileInputSpikeDriver::FileInputSpikeDriver(const char * NewFileName) : FileName(NewFileName), Currentline(1L){
	this->Finished = false;
...
void FileInputSpikeDriver::LoadInputs(EventQueue * Queue, Network * Net) {
	if (this->Handler){
		int ninputs,i;

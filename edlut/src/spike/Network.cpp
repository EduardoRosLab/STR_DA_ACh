...
void Network::FindInConnections(){
	if(this->ninters>0){
		...
		unsigned int * NumberOfInputsWithoutPostSynapticLearning = (unsigned int *) new unsigned int [this->nneurons]();
		unsigned int * InputsLeftWithoutPostSynapticLearning = (unsigned int *) new unsigned int [this->nneurons];
		...
		for (unsigned int wcindex; wcindex<this->nwchanges; ++wcindex){
			delete [] InputsLeftWithPostSynapticLearning[wcindex];
...
Network::Network(const std::list<NeuronLayerDescription> & neuron_layer_list,
				 ...
				 EventQueue * Queue, int numberOfQueues)  :
...
void Network::SaveWeights(const char *wfile) {
	FILE *fh;
	int connind;
	fh=fopen(wfile,"wt");
	if(fh){
